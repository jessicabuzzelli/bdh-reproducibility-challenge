...
with open("run_models.ps1", "w") as f:
    for dataset in ["support", "gbsg", "metabric"]:
        ...
        for t in range(num_trials/10):

            for s in range(1, 11):
                train_string = f"python main.py --save_model --save_log --dataset {dataset} --path data/{dataset} --split {s} --seed 1 --model_config_file data/hp_configs/{dataset}__rec_mlp__{t}__model.json --train_config_file data/hp_configs/{dataset}__rec_mlp__{t}__train.json > C:/Users/jbuzz/repos/bdh-reproducibility-challenge/SODEN/output/{dataset}/split{s}/model_{t}_eval.json"
